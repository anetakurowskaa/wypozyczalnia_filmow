...
<?php
    ...
    $emailr = filtrowanie($_POST['emailr']);
		...
		if (mysql_num_rows(mysql_query("SELECT `email` FROM `uzytkownicy` WHERE `email` = '".$emailr."';")) == 0)
		{
      // sprawdzenie czy hasła są takie same
			if ($haslor1 == $haslor2) 
			{
				mysql_query("INSERT INTO `uzytkownicy` (`haslo`, `email`)
					VALUES ('".sha1($haslor1)."', '".$emailr."');");
...
<form method="POST" action="rejestracja.php">
  E-mail: <input type="text" name="emailr">
	...
	<input type="submit" value="Utwórz konto" name="rejestracja"class="btn btn-primary">
	</form>
